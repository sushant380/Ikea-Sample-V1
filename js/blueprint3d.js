...
(function (BP3D) {
    ...
    (function (Items) {
        var GroupItem = (function () {
            function GroupItem(group,model,metadata,type,position, rotation, scale) {
                this.group=group;
                this.model=model;
                ...
                this.metadata=metadata;
                var scope=this;
                this.loader.load(metadata.model,function(geometry, materials){
                      var item = new (BP3D.Items.Factory.getClass(scope.type))(scope.model, scope.metadata, geometry, new THREE.MeshFaceMaterial(materials), position, rotation, scale);

                //item.fixed = fixed || false;
                    var x=scope.metadata.position[0]/Math.PI;
                    var y=scope.metadata.position[1]/Math.PI;
                    var z=scope.metadata.position[2]/Math.PI;
                    var vector3=new THREE.Vector3(x,y,z);
                    item.position.copy(vector3);
                    scope.group.add(item);
                    
                });

            };
            GroupItem.prototype.loadItem=function(geometry, materials){
...
(function (BP3D) {
    ...
    (function (Items) {
         ...
         * e.g. at walls or on the floor.
         */
        var CompositeItem = (function (_super) {
            __extends(CompositeItem, _super);
            /** Constructs an item.
             * @param model TODO
             * @param metadata TODO
             * @param geometry TODO
             ...
             * @param scale TODO
             */
            function CompositeItem(model, metadata, position, rotation, scale, parentScope) {
                _super.call(this);
                var scope=this;
                this.model=model;
                this.metadata = metadata;
                ...
                var loaderCallback = function (group) {
                    scope.castShadow=group.castShadow;
                    for(var m=0;m<group.children.length;m++){
                        group.children[m].position.y=group.children[m].position.y-group.position.y;
                        var children=group.children[m].clone();

                        /*var textureLoader = new THREE.TextureLoader();
                        textureLoader.load( 'models/js/'+children.name+'.jpg',
                        function(texture){
                         ...
                         /*var floorTexture = THREE.ImageUtils.loadTexture('models/js/'+children.name+'.jpg');
                        ...
                        children.material=floorMaterialTop;*/
                        scope.add(children);  

                    }
                    
                    scope.scale=group.scale;
                    scope.userData=group.userData;
                    scope.halfSize = scope.objectHalfSize();
                    scope.initObject();
                    parentScope.itemLoadedCallbacks.fire(scope);
                    if (rotation) {
                    scope.rotation.y = rotation;
                }
                if (scale != null) {
                    scope.setScale(scale.x, scale.y, scale.z);
                }
                };
               var loader=new THREE.ObjectLoader();
               loader.load(metadata.model,loaderCallback);
                   /*if(metadata.children){
                        var loadedModules=1;
                        for(var i=0;i<metadata.children.length;i++){
                            var child=metadata.children[i];
                            var filename=child.model;
                            var type=child.type;
                            new BP3D.Items.GroupItem(this,model,child,type);
                             var loaderCallback = function (geometry, materials) {
                                var item = new (BP3D.Items.Factory.getClass(type))(scope.model, child, geometry, new THREE.MeshFaceMaterial(materials), position, rotation, scale);

                                //item.fixed = fixed || false;
                                var m = new THREE.Matrix4(child.matrix[0],child.matrix[1],child.matrix[2],child.matrix[3],child.matrix[4],child.matrix[5],child.matrix[6],child.matrix[7],child.matrix[8],child.matrix[9],child.matrix[10],child.matrix[11],child.matrix[12],child.matrix[13],child.matrix[14],child.matrix[15]);
                               

                               // var buffergeometry = new THREE.BufferGeometry();

                                //buffergeometry.addAttribute( 'position', new THREE.BufferAttribute( new Float32Array( geometry.vertices ), 3 ) );
                                item.position.getPositionFromMatrix( m );
                                scope.add(item);
                               // item.initObject();
                                
                                if(loadedModules==metadata.children.length){
                                    scope.halfSize = scope.objectHalfSize();
                                    scope.initObject();
                                    parentScope.itemLoadedCallbacks.fire(scope);
                                }
            ...
            ;
            /** */
            CompositeItem.prototype.remove = function () {
                this.scene.removeItem(this);
            };
            
            /** */
            CompositeItem.prototype.resize = function (height, width, depth) {
                var x = width / this.getWidth();
                var y = height / this.getHeight();
                var z = depth / this.getDepth();
                this.setScale(x, y, z);
            ...
            /** */
            CompositeItem.prototype.setScale = function (x, y, z) {
                var scaleVec = new THREE.Vector3(x, y, z);
                this.halfSize.multiply(scaleVec);
                scaleVec.multiply(this.scale);
                this.scale.set(scaleVec.x, scaleVec.y, scaleVec.z);
            ...
            /** */
            CompositeItem.prototype.setFixed = function (fixed) {
                this.fixed = fixed;
            };
            /** */
            CompositeItem.prototype.removed = function () {
            };
            /** on is a bool */
            CompositeItem.prototype.updateHighlight = function () {
                var on = this.hover || this.selected;
                this.highlighted = on;
                var hex = on ? this.emissiveColor : 0x000000;
                /*this.material.materials.forEach(function (material) {
            ...
            /** */
            CompositeItem.prototype.mouseOver = function () {
                this.hover = true;
                this.updateHighlight();
            };
            ;
            /** */
            CompositeItem.prototype.mouseOff = function () {
                this.hover = false;
                this.updateHighlight();
            };
            ;
            /** */
            CompositeItem.prototype.setSelected = function () {
                this.selected = true;
                this.updateHighlight();
            };
            ;
            /** */
            CompositeItem.prototype.setUnselected = function () {
                this.selected = false;
                this.updateHighlight();
            };
            ;
            /** intersection has attributes point (vec3) and object (THREE.Mesh) */
            CompositeItem.prototype.clickPressed = function (intersection) {
                this.dragOffset.copy(intersection.point).sub(this.position);
            };
            ;
            /** */
            CompositeItem.prototype.clickDragged = function (intersection) {
                if (intersection) {
                    this.moveToPosition(intersection.point.sub(this.dragOffset), intersection);
                }
            };
            ;
            /** */
            CompositeItem.prototype.rotate = function (intersection) {
                if (intersection) {
                    var angle = BP3D.Core.Utils.angle(0, 1, intersection.point.x - this.position.x, intersection.point.z - this.position.z);
                    var snapTolerance = Math.PI / 16.0;
                    // snap to intervals near Math.PI/2
            ...
            /** */
            CompositeItem.prototype.moveToPosition = function (vec3, intersection) {
                this.position.copy(vec3);
            };
            /** */
            CompositeItem.prototype.clickReleased = function () {
                if (this.error) {
                    this.hideError();
                }
            };
             ...
             * for passing intersection to clickPressed and clickDragged
             */
            CompositeItem.prototype.customIntersectionPlanes = function () {
                return [];
            };
            /**
             * returns the 2d corners of the bounding polygon
             ...
             * TODO: handle rotated objects better!
             */
            CompositeItem.prototype.getCorners = function (xDim, yDim, position) {
                position = position || this.position;
                var halfSize = this.halfSize.clone();
                var c1 = new THREE.Vector3(-halfSize.x, 0, -halfSize.z);
                var c2 = new THREE.Vector3(halfSize.x, 0, -halfSize.z);
            ...
            /** */
            CompositeItem.prototype.showError = function (vec3) {
                vec3 = vec3 || this.position;
                if (!this.error) {
                    this.error = true;
                    this.errorGlow = this.createGlow(this.errorColor, 0.8, true);
            ...
            /** */
            CompositeItem.prototype.hideError = function () {
                if (this.error) {
                    this.error = false;
                    this.scene.remove(this.errorGlow);
                }
            ...
            /** */
            CompositeItem.prototype.objectHalfSize = function () {
                var objectBox = new THREE.Box3();
                objectBox.setFromObject(this);
                return objectBox.max.clone().sub(objectBox.min).divideScalar(2);
            };
            /** */
            CompositeItem.prototype.createGlow = function (color, opacity, ignoreDepth) {
                ignoreDepth = ignoreDepth || false;
                opacity = opacity || 0.2;
                var glowMaterial = new THREE.MeshBasicMaterial({
                    color: color,
                ...
                var objectBox = new THREE.Box3();
                objectBox.setFromObject(this);
                var geometry = new THREE.BoxGeometry( objectBox.width,objectBox.height,objectBox.depth);
            ...
            CompositeItem.prototype.openItem=function(){
                var doors=[];
                for(var m=0;m<this.children.length;m++){
                   if(this.children[m].name==="door_left"){
                        doors.push(this.children[m]);
                   }
                   if(this.children[m].name==="door_right"){
                        doors.push(this.children[m]);
                   }
                }
                        ...
                        function frame(){
                            if(angle1===90 && angle2===90){
                                ...
                                if(angle1<=90){
                                    var newRotation = new THREE.Euler( 0, (-angle1) * 0.017453292519943295, 0);
                                    doors[0].rotation.copy( newRotation );
                                    doors[0].updateMatrixWorld( true );
                                    angle1++;
                                }
                                if(angle2>=90){
                                    var newRotation = new THREE.Euler( 0, (-angle2) * 0.017453292519943295, 0);
                                    doors[1].rotation.copy( newRotation );
                                    doors[1].updateMatrixWorld( true );
                                    angle2--;
                                }
                                scope.model.scene.needsUpdate=true;
                            }
                        }
                    
            };
            CompositeItem.prototype.closeItem=function(){
                var doors=[];
                for(var m=0;m<this.children.length;m++){
                   if(this.children[m].name==="door_left"){
                        doors.push(this.children[m]);
        ...
        Items.CompositeItem = CompositeItem;
    })(Items = BP3D.Items || (BP3D.Items = {}));
})(BP3D || (BP3D = {}));
...
(function (BP3D) {
    ...
    (function (Items) {
         ...
         * A Floor Item is an entity to be placed related to a floor.
         */
        var CompositeFloorItem = (function (_super) {
            __extends(CompositeFloorItem, _super);
            function CompositeFloorItem(model, metadata, position, rotation, scale,parentScope) {
                _super.call(this, model, metadata, position, rotation, scale,parentScope);
            }
            ;
            /** */
            CompositeFloorItem.prototype.placeInRoom = function () {
                if (!this.position_set) {
                    var center = this.model.floorplan.getCenter();
            ...
            ;
            /** Take action after a resize */
            CompositeFloorItem.prototype.resized = function () {
                this.position.y = 0;
            };
            /** */
            CompositeFloorItem.prototype.moveToPosition = function (vec3, intersection) {
                // keeps the position in the room and on the floor
                if (!this.isValidPosition(vec3)) {
                    this.showError(vec3);
                    return;
            ...
            /** */
            CompositeFloorItem.prototype.isValidPosition = function (vec3) {
                var corners = this.getCorners('x', 'z', vec3);
                // check if we are in a room
                var rooms = this.model.floorplan.getRooms();
                var isInARoom = false;
            ...
            return CompositeFloorItem;
        })(Items.CompositeItem);
        Items.CompositeFloorItem = CompositeFloorItem;
    })(Items = BP3D.Items || (BP3D.Items = {}));
})(BP3D || (BP3D = {}));
/// <reference path="../../lib/three.d.ts" />
/// <reference path="../core/utils.ts" />
                ...
                if(itemType===10){
                   // this.itemLoadingCallbacks.fire();
                    for(var i=0;i<itemsData.length;i++){
                        if(itemsData[i].name===metadata.itemName){
                            var item = new (BP3D.Items.Factory.getClass(itemType))(scope.model, itemsData[i], position, rotation, scale,this);
                            item.fixed = fixed || false;
        ...
        Three.Controller = function (three, model, camera, element, controls, hud) {
            ...
            function updateIntersections() {
                ...
                if (intersects.length > 0) {
                    if(intersects[0].object.parent instanceof BP3D.Items.CompositeItem){
            ...
            this.getIntersections = function (vec2, objects, filterByNormals, onlyVisible, recursive, linePrecision) {
                ...
                if (objects instanceof Array) {
                    ...
                    for(var m=0;m<objects.length;m++){
                        if(objects[m] instanceof BP3D.Items.CompositeItem){
                           intersections=raycaster.intersectObjects(objects[m].children, recursive);
    ...
    (function (Three) {
        Three.Main = function (model, element, canvasElement, opts) {
            ...
            function init() {
                ...
                camera = new THREE.CombinedCamera(window.innerWidth / 2, window.innerHeight / 2, 70, 1, 10000, - 500, 1000);
...
(function (BP3D) {
    ...
    var RpdConverter = (function () {
        /** Creates an instance.
         * @param options The initialization options.
         */
        function RpdConverter(rpd) {
           
        };
        RpdConverter.prototype.loadRoom=function(rpd){
            this.floorplan = new BP3D.Model.Floorplan();
            var roomData=rpd.RoomBaseData;
            var coordinates=roomData.coords;
            var corners=[];
             
           
            for(var m=0;m<coordinates.length;m++){
                var x=coordinates[m].x*100;
                var y=-(coordinates[m].y*100);
                var corner=this.floorplan.newCorner(x,y,BP3D.Core.Utils.guid());
                console.log(x,y);
                
                
            }
            //rearrange corners;
            var c4=this.floorplan.corners[4];
            var c5=this.floorplan.corners[5];
            var c6=this.floorplan.corners[6];
            var c7=this.floorplan.corners[7];
            this.floorplan.corners[4]=c6;
            this.floorplan.corners[6]=c4;

            for(var k=0;k<this.floorplan.corners.length;k++){
            // console.log(this.floorplan.corners[k]);   
                if(k===7){
                    var wall=this.floorplan.newWall(this.floorplan.corners[k],this.floorplan.corners[0]);
                    
                }else{
                    var wall=this.floorplan.newWall(this.floorplan.corners[k],this.floorplan.corners[k+1]);    
                    
                }                
            }
            var rawitems=rpd.AllItemData;
            var items=[];
            for(var name in rawitems){
                var item=rawitems[name];
                ...
                switch(itemType){
                    case 'OnFloor':
                        var floorItem={
                            item_name: item.Item,
                            item_type: 10,
                            model_url: 'models/js/50215475.js',
                            xpos: item.Pos.x*100,
                            ypos: item.Pos.y*100,
                            zpos: item.Pos.z*100,
                            rotation: item.Ori.y,
        ...
        RpdConverter.prototype.rpdToJson=function(rpd){
            var rpdJSON = new Object()
            var decodedRpd=window.decodeURIComponent(rpd);
            var t = this.rpdStringToArray(decodedRpd, "&&")
    
            for(var i = 0; i<t.length; i++) {
                var r = this.rpdStringToArray(t[i], "==")
            
                if(r[0] == "RoomBaseData") {    // get room data
                    rpdJSON[r[0]]=this.noNameParamsToJSON(r[1])
                }

                else if(r[0] == "AllItemData") {
                    
                    rpdJSON[r[0]]=this.noneUniqueParamsToJSON(r[1], 'item')
                    
                }
                
                else if(r[0] == "AllWallData") {
                    rpdJSON[r[0]]=this.noneUniqueParamsToJSON(r[1], 'wall')
                    
                }
                
                else {
                    rpdJSON[r[0]]=this.uniqueParamsToJSON(r[1])
                }
            }
            console.log(rpdJSON);
            return rpdJSON;
        };
        RpdConverter.prototype.rpdStringToArray=function(rpdString, divideTooken) {
            return rpdString.split(divideTooken)
        };
        RpdConverter.prototype.uniqueParamsToJSON=function(a) { // assuming the params are unique for part
            var tempObj = new Object()
            var r = this.rpdStringToArray(a, "&")
                for(var i = 0; i<r.length; i++) {
                    var b = this.rpdStringToArray(r[i], "=")
                    tempObj[b[0]]=b[1]
                }
            return tempObj
        };
        RpdConverter.prototype.noNameParamsToJSON=function(a) { // need to return an object with arrays which is needed positions in room base data
            var r = this.rpdStringToArray(a, "&")
            ...
            var coordArray = new Array()
                ...
                if(this.checkIfCoordString(r[i])) {  
                    coordArray.push(this.createCoordsArray(r[i]))
         ...
         RpdConverter.prototype.createCoordsArray=function(str) {
            var coordTemp = new THREE.Vector3()         
            var coordAr = this.rpdStringToArray(str, " ")
            coordTemp.x = this.convertRPDCoordsToUnit(coordAr[0])    
            coordTemp.y = this.convertRPDCoordsToUnit(coordAr[1])
            coordTemp.z = this.convertRPDCoordsToUnit(coordAr[2])

            return coordTemp
        };
        RpdConverter.prototype.checkIfCoordString=function(a) { // check if the string matches coords pattern with punctuations
        //  var pattern = new RegExp("^[-]*\d+[.]\d+\s[-]*\d+[.]\d+\s[-]*\d+[.]\d+")
            pattern =  /^[-]*\d+[.]*\d*\s[-]*\d+[.]*\d*\s[-]*\d+[.]*\d*$/ // /^[-]*\d+[.]\d+\s[-]*\d+[.]\d+\s[-]*\d+[.]\d+/
            return pattern.test(a)
        };
        RpdConverter.prototype.convertRPDCoordsToUnit=function(a) {

            return parseFloat(this.convertRPDCoordsToMM(a)/1000)
        };
        RpdConverter.prototype.convertRPDCoordsToMM=function(a) {
            return a/80
        };
         RpdConverter.prototype.noneUniqueParamsToJSON=function(a, tag) { // need to return an object with arrays which is needed for items
    var r = this.rpdStringToArray(a, ";")
    var retObj =  new Object()

        for(var i = 0; i<r.length; i++) {
                ////console.log(r[i])
                var params=this.rpdStringToArray(r[i], "&")
                retObj[tag+i]={}
                for(var j = 0; j<params.length; j++) {
                    var valuePair=this.rpdStringToArray(params[j], "=")
                    
                    //console.log(valuePair[0]+" "+valuePair[1])
                
                    if(valuePair[0].indexOf("Ori")>-1 ) retObj[tag+i][valuePair[0]]=this.createOriArray(valuePair[1]) // orientation coords in degree
                        
                    else if(this.checkIfCoordString(valuePair[1]) && valuePair[0]!="Scl") {
                        retObj[tag+i][valuePair[0]]=this.createCoordsArray(valuePair[1])                     
                    }
                        
                    else if(valuePair[0].indexOf("Pos")>-1 ) retObj[tag+i][valuePair[0]]=this.convertRPDCoordsToUnit(valuePair[1])   
...
//              retObj[tag+i]
    ...
    RpdConverter.prototype.createOriArray=function (str) {
    ...
    BP3D.RpdConverter = RpdConverter;
})(BP3D || (BP3D = {}));
var BP3D;
(function (BP3D) {
    /** Blueprint3D core application. */
